...
actor LMS {
  ...
  // === STUDENT DASHBOARD ===
  public func getStudentDashboard(token: Text): async Result.Result<{
    enrolled_courses: Nat;
    completed_courses: Nat;
    pending_assignments: Nat;
    certificates: Nat;
    half_progress_courses: [Course];
    upcoming_deadlines: [Assignment];
    recent_finished_courses: [Course];
  }, Text> {
      ...
      case (#ok(user)) {
        ...
        switch (studentRecord) {
          ...
          case (?student) {
            ...
            let halfProgressEnrollments = Array.filter(studentEnrollments, func(e: Enrollment): Bool { 
              e.progress_percentage >= 40 and e.progress_percentage <= 60 
            });
            let halfProgressCourses = Array.mapFilter(halfProgressEnrollments, func(e: Enrollment): ?Course {
            ...
            let currentTime = Time.now();
            let upcomingDeadlines = Array.filter(studentAssignments, func(a: Assignment): Bool {
            ...
            let recentFinishedEnrollments = Array.filter(studentEnrollments, func(e: Enrollment): Bool {
            ...
            let recentFinishedCourses = Array.mapFilter(recentFinishedEnrollments, func(e: Enrollment): ?Course {
              let classObj = Array.find(classes, func(cl: Class): Bool { cl.class_id == e.class_id });
              switch (classObj) {
                case null { null };
                case (?cl) {
  ...
  // === MY COURSES ===
  public func getMyCourses(token: Text): async Result.Result<[{
    course: Course;
    progress: Nat;
    enrollment_status: Text;
  }], Text> {
    switch (await validateStudent(token)) {
      ...
      case (#ok(user)) {
        ...
        switch (studentRecord) {
          ...
          case (?student) {
            let studentEnrollments = Array.filter(enrollments, func(e: Enrollment): Bool { e.student_id == student.student_id });
            let coursesWithProgress = Array.mapFilter(studentEnrollments, func(e: Enrollment): ?{course: Course; progress: Nat; enrollment_status: Text} {
              let classObj = Array.find(classes, func(cl: Class): Bool { cl.class_id == e.class_id });
              switch (classObj) {
                case null { null };
                case (?cl) {
      ...
      case (#ok(user)) {
        ...
        switch (studentRecord) {
          ...
          case (?student) {
            ...
            let sortedAssignments = Array.sort(studentAssignments, func(a: Assignment, b: Assignment): {#less; #equal; #greater} {
              ...
              if (aOrder < bOrder) { #less }
              else if (aOrder > bOrder) { #greater }
              else { #equal };
            });
      ...
      case (#ok(user)) {
        ...
        switch (studentRecord) {
          ...
          case (?student) {
            ...
            let recommendedCourses = Array.filter(courses, func(c: Course): Bool {
  ...
  public func getInstructorDashboard(token: Text): async Result.Result<{
    total_students: Nat;
    total_active_courses: Nat;
    total_revenue: Nat;
    average_rating: Float;
    top_performing_course: ?Course;
    newest_course: ?Course;
    all_courses: [{course: Course; student_count: Nat; rating: Float}];
  }, Text> {
      ...
      case (#ok(user)) {
        ...
        switch (instructorRecord) {
          ...
          case (?instructor) {
            ...
            let totalStudents = Array.foldLeft(instructorCourses, 0, func(acc: Nat, course: Course): Nat {
              acc + course.student_count
            });
            
            // Calculate average rating
            let totalRating = Array.foldLeft(instructorCourses, 0.0, func(acc: Float, course: Course): Float {
              acc + course.rating
            });
            ...
            let topCourse = Array.foldLeft(instructorCourses, null: ?Course, func(acc: ?Course, course: Course): ?Course {
              switch (acc) {
                case null { ?course };
                case (?current) { if (course.student_count > current.student_count) ?course else ?current };
              };
            });
            
            // Find newest course
            let newestCourse = Array.foldLeft(instructorCourses, null: ?Course, func(acc: ?Course, course: Course): ?Course {
              switch (acc) {
                case null { ?course };
            ...
            let coursesWithStats = Array.map(instructorCourses, func(c: Course): {course: Course; student_count: Nat; rating: Float} {
              {course = c; student_count = c.student_count; rating = c.rating};
            });
  ...
  // === ADMIN DASHBOARD ===
  public func getAdminDashboard(token: Text): async Result.Result<{
    student_count: Nat;
    instructor_count: Nat;
    course_count: Nat;
    revenue: Nat;
    role_distribution: {students: Nat; instructors: Nat; admins: Nat};
    recent_events: {new_students: Nat; new_instructors: Nat; new_courses: Nat};
  }, Text> {
    switch (await validateAdmin(token)) {
      ...
      case (#ok(user)) {
        ...
        let newStudents = Array.size(Array.filter(users, func(u: User): Bool {
          u.role == "student" and createdWithinLast24Hours(u)
        }));

        let newInstructors = Array.size(Array.filter(users, func(u: User): Bool {
  ...
  public func updateStudent(s: Student): async () {
    students := Array.map(students, func(x: Student): Student { 
  ...
  public func updateLecturer(l: Lecturer): async () {
    lecturers := Array.map(lecturers, func(x: Lecturer): Lecturer { 
  ...
  public func updateCourse(c: Course): async () {
    courses := Array.map(courses, func(x: Course): Course { 
  ...
  public func updateClass(c: Class): async () {
    classes := Array.map(classes, func(x: Class): Class { 
  ...
  public func updateEnrollment(e: Enrollment): async () {
    enrollments := Array.map(enrollments, func(x: Enrollment): Enrollment { 
  ...
  public func updateAssignment(a: Assignment): async () {
    assignments := Array.map(assignments, func(x: Assignment): Assignment { 
  ...
  public func updateCertificate(c: Certificate): async () {
    certificates := Array.map(certificates, func(x: Certificate): Certificate { 
  ...
  public func updateReview(r: Review): async () {
    reviews := Array.map(reviews, func(x: Review): Review { 
  ...
  // === DISCUSSIONS ===
  public func getDiscussions(token: Text): async Result.Result<[{
      ...
      case (#ok(user)) {
        ...
        switch (studentRecord) {
          ...
          case (?student) {
            ...
            let relevantDiscussions = Array.filter(discussions, func(d: Discussion): Bool {
              Array.find(enrolledClasses, func(classId: Text): Bool { classId == d.class_id }) != null
            });
            
            let discussionsWithCourse = Array.mapFilter(relevantDiscussions, func(d: Discussion): ?{discussion: Discussion; course_name: Text; hours_ago: Nat} {
  ...
  // === CLASSMATES ===
  public func getClassmates(token: Text): async Result.Result<[{
    course_name: Text;
    classmates: [{name: Text; email: Text; profile_picture: Text}];
  }], Text> {
    switch (await validateStudent(token)) {
      case (#err(msg)) { #err(msg) };
      case (#ok(user)) {
        ...
        switch (studentRecord) {
          ...
          case (?student) {
            ...
            let classmatesByCourse = Array.mapFilter(studentEnrollments, func(enrollment: Enrollment): ?{course_name: Text; classmates: [{name: Text; email: Text; profile_picture: Text}]} {
              ...
              switch (classObj) {
                ...
                case (?cl) {
                  ...
                  switch (course) {
                    case null { null };
                    case (?c) {
                      let otherEnrollments = Array.filter(enrollments, func(e: Enrollment): Bool {
                        e.class_id == enrollment.class_id and e.student_id != student.student_id
                      });
                      
                      let classmates = Array.mapFilter(otherEnrollments, func(e: Enrollment): ?{name: Text; email: Text; profile_picture: Text} {
  ...
  // === CERTIFICATES ===
  public func getStudentCertificates(token: Text): async Result.Result<[{
    course_title: Text;
      ...
      case (#ok(user)) {
        ...
        switch (studentRecord) {
          ...
          case (?student) {
            ...
            let certificatesWithCourse = Array.mapFilter(studentCertificates, func(cert: Certificate): ?{course_title: Text; completion_date: Text; certificate_id: Text} {
  ...
  // === USER SETTINGS ===
  public func getUserSettings(token: Text): async Result.Result<{
    first_name: Text;
    last_name: Text;
    email: Text;
    bio: Text;
  ...
  public func updateUserSettings(token: Text, settings: {
    first_name: Text;
    last_name: Text;
    email: Text;
    bio: Text;
  ...
  public func createCourseWithMaterials(token: Text, courseData: {
    course_name: Text;
    course_description: Text;
    duration_days: Nat;
    language: Text;
  ...
  // === QUIZ BUILDER ===
  public func createQuiz(token: Text, quizData: {
    course_id: Text;
    title: Text;
    questions: [{
      question_text: Text;
      ...
      case (#ok(user)) {
        ...
        switch (instructorRecord) {
          ...
          case (?instructor) {
            ...
            let coursesWithQuizzes = Array.map(instructorCourses, func(course: Course): {course: Course; quiz_count: Nat} {
  ...
  // === QUIZ CHECKER DETAIL ===
  public func getQuizSubmissions(token: Text, quizId: Text): async Result.Result<[{
    ...
    switch (await validateInstructor(token)) {
      ...
      case (#ok(user)) {
        ...
        let submissionsWithStudent = Array.mapFilter(submissions, func(sub: QuizSubmission): ?{submission: QuizSubmission; student_name: Text; needs_grading: Bool} {
  ...
  public func getStudentsInCourses(token: Text): async Result.Result<[{
    student_name: Text;
    student_email: Text;
    profile_picture: Text;
    courses: Text; // comma-separated course names
    student_id: Text;
  }], Text> {
    switch (await validateInstructor(token)) {
      case (#err(msg)) { #err(msg) };
      case (#ok(user)) {
        ...
        switch (instructorRecord) {
          ...
          case (?instructor) {
            ...
            let relevantEnrollments = Array.filter(enrollments, func(e: Enrollment): Bool {
              let classObj = Array.find(classes, func(cl: Class): Bool { cl.class_id == e.class_id });
              switch (classObj) {
                case null { false };
                case (?cl) {
            ...
            let studentEnrollments = Array.foldLeft(relevantEnrollments, [], func(acc: [(Text, [Enrollment])], enrollment: Enrollment): [(Text, [Enrollment])] {
              switch (Array.find(acc, func(item: (Text, [Enrollment])): Bool { item.0 == enrollment.student_id })) {
                case null { Array.append(acc, [(enrollment.student_id, [enrollment])]) };
                case (?existing) {
            ...
            let studentsWithCourses = Array.mapFilter(studentEnrollments, func(item: (Text, [Enrollment])): ?{student_name: Text; student_email: Text; profile_picture: Text; courses: Text; student_id: Text} {
              ...
              switch (student) {
                ...
                case (?s) {
                  ...
                  switch (studentUser) {
                    case null { null };
                    case (?u) {
                      let courseNames = Array.mapFilter(studentEnrollments, func(e: Enrollment): ?Text {
                        let classObj = Array.find(classes, func(cl: Class): Bool { cl.class_id == e.class_id });
                        switch (classObj) {
                          case null { null };
                          case (?cl) {
                      ...
                      let coursesText = Array.foldLeft(courseNames, "", func(acc: Text, courseName: Text): Text {
  ...
  public func getUsersForManagement(token: Text, nameFilter: Text): async Result.Result<[{
    user_id: Text;
    name: Text;
    email: Text;
    role: Text;
    ...
    switch (await validateAdmin(token)) {
      ...
      case (#ok(user)) {
        ...
        let managementUsers = Array.map(filteredUsers, func(u: User): {user_id: Text; name: Text; email: Text; role: Text; status: Text; profile_picture: Text} {
          {
            user_id = u.user_id;
  ...
  // === STUDENT REPORTING ===
  public func reportStudent(token: Text, reportData: {
    student_id: Text;
    course_id: Text;
    problem_description: Text;
  }): async Result.Result<Text, Text> {
  ...
  public func getStudentReports(token: Text): async Result.Result<[{
    ...
    switch (await validateAdmin(token)) {
      case (#err(msg)) { #err(msg) };
      case (#ok(user)) {
        let reportsWithDetails = Array.mapFilter(student_reports, func(report: StudentReport): ?{report: StudentReport; student_name: Text; student_email: Text; profile_picture: Text; course_name: Text} {
      ...
      case (#ok(user)) {
        switch (Array.find(student_reports, func(r: StudentReport): Bool { r.report_id == reportId })) {
          ...
          case (?report) {
            ...
            student_reports := Array.map(student_reports, func(r: StudentReport): StudentReport {
                ...
                case (?s) {
                  ...
                  switch (studentUser) {
                    case (?u) {
                      ...
                      users := Array.map(users, func(user: User): User {
